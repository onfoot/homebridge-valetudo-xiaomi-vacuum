...
class ValetudoXiaomiVacuum {
    ...
    constructor(log, config) {
        ...
        this.just_on_off = config['just-on-off'] == true;
    ...
    setFanSpeed (value, callback) {
        ...
        this.sendJSONRequest('http://' + this.ip + '/api/v2/robot/capabilities/FanSpeedControlCapability/preset', 'PUT', {name: value}, true)
        .then((response) => {
            callback();
            this.updateStatus (true);
        })
    ...
    doFind (state, callback) {
        ...
        if (state) {
            log.error('Executing vacuum find');
            this.identify(() => {
                callback(null);
                setTimeout(() => {
    ...
    identify (callback) {
        ...
        this.sendJSONRequest('http://' + this.ip + '/api/v2/robot/capabilities/LocateCapability', 'PUT', {
            action: "locate"
        }, true)
    ...
    goHome (state, callback) {
        ...
        if (state) {
            ...
            this.sendJSONRequest('http://' + this.ip + '/api/v2/robot/capabilities/BasicControlCapability', 'PUT', {
                action: "home"
            }, true)
                .then((response) => {
