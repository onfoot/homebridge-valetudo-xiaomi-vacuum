...
class ValetudoXiaomiVacuum {
  constructor(log, config) {
    ...
    this.re_mode = config['legacy-mode'] === true;

    if (!this.re_mode) {
      this.statusUrl = ValetudoXiaomiVacuum.statusUrlModern;

      this.parseStatus = ValetudoXiaomiVacuum.parseStatusModern;
      this.getSpeedValue = ValetudoXiaomiVacuum.getSpeedValueModern;
      ...
      this.statusUrl = ValetudoXiaomiVacuum.statusUrlLegacy;
      this.parseStatus = ValetudoXiaomiVacuum.parseStatusLegacy;
      this.getSpeedValue = ValetudoXiaomiVacuum.getSpeedValueLegacy;
  ...
  static statusUrlModern(ip) {
    return `http://${ip}/api/v2/robot/state/attributes`;
  }

  static statusUrlLegacy(ip) {
    return `http://${ip}/api/current_status`;
  }

  static parseStatusModern(response) {
    const simpleStatus = {};
  ...
  static parseStatusLegacy(response) {
    return response;
  }
  ...
  getChargingModern(callback) {
    this.getStatus(false, (error) => {
      if (error) {
        callback(error);
      } else if (this.current_status.battery_status === ValetudoXiaomiVacuum.BATTERY.CHARGING) {
  ...
  getChargingLegacy(callback) {
    this.getStatus(false, (error) => {
      if (error) {
        callback(error);
      } else if (this.current_status.state === ValetudoXiaomiVacuum.STATES_LEGACY.CHARGING) {
        this.log.debug('CHARGING!');
        callback(null, Characteristic.ChargingState.CHARGING);
      } else if (
        this.current_status.state === ValetudoXiaomiVacuum.STATES_LEGACY.CHARGER_DISCONNECTED
        || this.current_status.state === ValetudoXiaomiVacuum.STATES_LEGACY.CHARGING_PROBLEM
      ) {
        callback(null, Characteristic.ChargingState.NOT_CHARGEABLE);
      } else {
        callback(null, Characteristic.ChargingState.NOT_CHARGING);
  ...
  async goHomeLegacy(state, callback) {
    ...
    if (state) {
      ...
      try {
        await this.sendJSONRequest({ url: `http://${this.ip}/api/drive_home`, method: 'PUT', raw_response: true });
        setTimeout(() => { callback(); this.updateStatus(true); }, 3000);
      } catch (e) {
        log.error(`Failed to execute go home: ${e}`);
        setTimeout(() => { callback(); this.updateStatus(true); }, 3000);
  ...
  async startCleaningLegacy(state, callback) {
    ...
    if (state) {
      ...
      try {
        await this.sendJSONRequest({ url: `http://${this.ip}/api/start_cleaning`, method: 'PUT', raw_response: true });
        setTimeout(() => { callback(); this.updateStatus(true); }, 3000);
      } catch (e) {
  ...
  async stopCleaningLegacy(callback) {
    ...
    this.getStatus(true, async (err) => {
      ...
      if (
        this.current_status.state === ValetudoXiaomiVacuum.STATES_LEGACY.IDLE
      ...
      try {
        await this.sendJSONRequest({ url: `http://${this.ip}/api/stop_cleaning`, method: 'PUT', raw_response: true });
        setTimeout(() => { callback(); this.updateStatus(true); }, 3000);
      } catch (e) {
        this.log.error(`Failed to stop cleaning: ${e}`);
        setTimeout(() => { callback(); this.updateStatus(true); }, 3000);
  ...
  async startSpotCleaning(state, callback) {
    ...
    if (state) {
      ...
      try {
        await this.sendJSONRequest({ url: `http://${this.ip}/api/spot_clean`, method: 'PUT', raw_response: true });
        setTimeout(() => { callback(); this.updateStatus(true); }, 3000);
      } catch (e) {
  ...
  isSpotCleaningModern(callback) {
    this.getStatus(false, (error) => {
      ...
      callback(null,
        this.current_status.cleaning_mode === ValetudoXiaomiVacuum.CLEANING_MODES.SPOT);
    });
  }
  ...
  updateStatusModern(forced = false) {
    ...
    this.getStatus(forced, (err) => {
      ...
      this.batteryService.updateCharacteristic(
        Characteristic.BatteryLevel, this.current_status.battery,
      );
      this.batteryService.updateCharacteristic(
        Characteristic.StatusLowBattery, this.current_status.battery < 10
      ...
      this.batteryService.updateCharacteristic(
        ...
        this.current_status.battery_status === ValetudoXiaomiVacuum.BATTERY.CHARGING
          ? Characteristic.ChargingState.CHARGING
          : Characteristic.ChargingState.NOT_CHARGING,
      );

      this.cleanService.updateCharacteristic(Characteristic.On,
        this.current_status.state === ValetudoXiaomiVacuum.STATES.CLEANING); // cleaning
      this.goHomeService.updateCharacteristic(Characteristic.On,
        this.current_status.state === ValetudoXiaomiVacuum.STATES.RETURNING); // driving home
      this.spotCleanService.updateCharacteristic(Characteristic.On,
        this.current_status.cleaning_mode
          === ValetudoXiaomiVacuum.CLEANING_MODES.SPOT); // spot cleaning
  ...
  updateStatusLegacy(forced = false) {
    ...
    this.getStatus(forced, (err) => {
      ...
      this.batteryService.updateCharacteristic(Characteristic.BatteryLevel,
        this.current_status.battery);
      this.batteryService.updateCharacteristic(Characteristic.StatusLowBattery,
        this.current_status.battery < 10
          ? Characteristic.StatusLowBattery.BATTERY_LEVEL_LOW
          : Characteristic.StatusLowBattery.BATTERY_LEVEL_NORMAL);

      this.batteryService.updateCharacteristic(Characteristic.ChargingState,
        this.current_status.state === ValetudoXiaomiVacuum.STATES_LEGACY.CHARGING
          ? Characteristic.ChargingState.CHARGING : Characteristic.ChargingState.NOT_CHARGING);

      this.cleanService.updateCharacteristic(Characteristic.On,
        this.current_status.state === ValetudoXiaomiVacuum.STATES_LEGACY.CLEANING);
      this.goHomeService.updateCharacteristic(Characteristic.On,
        this.current_status.state === ValetudoXiaomiVacuum.STATES_LEGACY.RETURNING_HOME);
      this.spotCleanService.updateCharacteristic(Characteristic.On,
        this.current_status.state === ValetudoXiaomiVacuum.STATES_LEGACY.SPOT_CLEANING);
  ...
  static getSpeedValueModern(preset) {
    switch (preset) {
  ...
  static getSpeedValueLegacy(preset) {
    switch (preset) {
